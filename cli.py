...
@click.option("-f","--file",type=click.Path(exists=True,dir_okay=False,readable=True),help="Read prompt from file.")
...
def run_cmd(client: OllamaClient, model:str, stream:bool, system:Optional[str], options:Optional[str], file:Optional[str], prompt:Optional[str]):
    ...
    if file:
        with open(file,"r",encoding="utf-8") as fh: prompt_text=fh.read()
    elif prompt: prompt_text=prompt
    else:
        if sys.stdin.isatty(): click.echo("Enter prompt, then Ctrl-D (macOS/Linux) or Ctrl-Z Enter (Windows):\n",err=True)
        prompt_text=sys.stdin.read()
    prompt_text=prompt_text.strip()
    if not prompt_text: click.echo("Error: Prompt is empty.",err=True); raise SystemExit(2)
    options_dict=None
    if options:
        try: options_dict=json.loads(options)
        except json.JSONDecodeError: click.echo("Error: --options must be valid JSON.",err=True); raise SystemExit(2)
    try:
        result=client.generate(model=model,prompt=prompt_text,stream=stream,system=system,options=options_dict)
    except OllamaError as e: click.echo(f"Error: {e}",err=True); raise SystemExit(1)
    if stream:
        for chunk in result: click.echo(chunk,nl=False)
        click.echo()
    else: click.echo(result)

def main(): cli(prog_name="ai-cli")
